...
"""
        ...
        Quito el filtro y hago todo large signal
...
Escalon_Planta_Digital_Recursivo = False
...
Amp_gain = 4.4    #no se usa se saca como (Ri + Rf)/Ri
...
if Bode_Planta_Analog == True:
    ...
    ax1.set_title('Magnitude Input to Output Plant Analog')
...
if Bode_Polo_Opamp_Analog == True:
    ...
    # ax1.semilogx (w/6.28, mag_s, 'g-', linewidth="1")
    ax1.set_title('Magnitude Only Sensor Analog')
...
if Bode_Sensor_Analog == True:
    ...
    # ax1.semilogx (w/6.28, mag_s, 'g-', linewidth="1")
    ax1.set_title('Magnitude Input to Output Sensor Analog')
...
if Bode_Planta_Digital == True:
    planta_vcc = lti_to_sympy(planta_dig_zoh) * vcc
    planta_dig_zoh_vcc = sympy_to_lti(planta_vcc)
    planta_dig_zoh_vcc = TransferFunction(planta_dig_zoh_vcc.num, planta_dig_zoh_vcc.den, dt=td)
    w, mag_zoh, phase_zoh = dbode(planta_dig_zoh, n = 10000)
    w, mag_vcc_zoh, phase_vcc_zoh = dbode(planta_dig_zoh_vcc, n = 10000)    

    fig, (ax1, ax2) = plt.subplots(2,1)

    ax1.semilogx(w/(2*np.pi), mag_zoh, 'y')
    ax1.semilogx(w/(2*np.pi), mag_vcc_zoh, 'c')        
    ax1.set_title(f'Plant Digital Bode ZOH Yellow no Vcc, Cyan with Vcc = {vcc}V')
...
if Bode_Sensor_Digital == True:
    ...
    ax1.semilogx(w/(2*np.pi), mag_zoh, 'y')
    ax1.semilogx(w/(2*np.pi), mag_vcc_zoh, 'c')        
    ax1.set_title(f'Sensor Digital Bode ZOH Yellow no Vcc, Cyan with Vcc = {vcc}V')
    ax1.set_xlabel('Frequency [Hz]')
...
if Escalon_Sensor_Digital == True:
    ...
    fig, ax = plt.subplots()
    ax.set_title('Step Sensor Digital ZOH')
...
############################################################
if Escalon_Planta_Digital_Recursivo == True:
    # ZOH
    b_planta = np.transpose(planta_dig_zoh.num)
    a_planta = np.transpose(planta_dig_zoh.den)
...
if Escalon_Sensor_Digital_Recursivo == True:
    ...
    fig, ax = plt.subplots()
    ax.set_title('Step Sensor Digital Recursiva ZOH')
    ax.set_ylabel('Tension del Sensor')
    ax.set_xlabel('Tiempo [s]')
    ax.grid()
    ax.plot(t, vout_plant_method2, 'y')    
...
if PID_Single_Stage == True:
    pid_tf_single = PID_int(b_pid_int, a_pid_int, 128)
    ...
    for i in range(1, len(vin_plant)):
        ...
        ########################################
        # DCM
        if vin_plant[i] > vout_line[i]:
            vin_plant_d[i] = vin_plant[i] - vout_line[i]
        else:
            vin_plant_d[i] = 0

        if d[i] != 0:
            Re[i] = 2 * Lm * max_pwm_pts /(Tpwm * d[i])
        else:
            Re[i] = 1e6

        vout_plant[i] = (vin_plant_d[i] * Rsense / (Rload + Rsense + Re[i])) * Amp_gain
