...
"""
        ...
        Con filtro entre plant_out y sensor
"""

##########################################################################
# Cuales son los Graficos que quiero mostrar por cuestiones de velocidad #
##########################################################################
Bode_Analog_Planta_Filtro_Sensor = True
Bode_Sensor_Digital = True
Polos_Ceros_Planta_Digital = False
...
#########################################
# Filtro colocado entre Planta y Sensor #
#########################################
fa = 1000
fb = 1000
a = 2 * np.pi * fa
b = 2 * np.pi * fb
Transf_Filter = a / (s + a) * b / (s + b)

Filter_out_sim = Transf_Filter.simplify()
print ('Filter_out: ')
print (Filter_out_sim)

Sensor_out = Plant_out_sim * Amp_gain * Filter_out_sim
...
filtro_TF = sympy_to_lti(Filter_out_sim)
print ("filtro con sympy:")
print (filtro_TF)
...
if Bode_Sensor_Digital == True:
    ...
    ax1.set_title(f'Sensor Digital Input {Vinput}V green - ZOH -')
...
if Escalon_Sensor_Digital == True:
    fig, ax = plt.subplots()
    ax.set_title(f'Step Sensor Digital {Vinput}V - ZOH -')
...
if Escalon_Sensor_Digital_Recursivo == True:
    fig, ax = plt.subplots()
    ax.set_title(f'Step Planta Digital Recursiva {Vinput}V - ZOH -')    
    ax.set_ylabel('Tension del Sensor')
    ax.set_xlabel('Tiempo [s]')
    ax.grid()
